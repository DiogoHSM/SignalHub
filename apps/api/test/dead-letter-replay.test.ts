...
describe("replayDeadLetterTelemetryJob", () => {
  ...
  it("validates session replay telemetry dead-letter jobs", async () => {
    const getDeadLetterJob = vi.fn(async () =>
      deadLetterJob({
        jobName: "replay",
        payload: {
          ...
          environmentId: "env_1",
          payload: {
            replay_id: "rpl_browser_1",
            started_at: "2026-06-01T12:00:00.000Z",
            ended_at: "2026-06-01T12:00:01.000Z",
            duration_ms: 1000,
            route: "/checkout",
            masked: true,
            events: [{ offset_ms: 0, type: "navigation", route: "/checkout", data: {} }]
          }
        }
      })
    );
    ...
    expect(enqueueReplay).toHaveBeenCalledWith(
      expect.objectContaining({
        kind: "replay",
        id: "rpl_job_1",
        payload: expect.objectContaining({ replay_id: "rpl_browser_1", masked: true })
      }),
      "dlj_1|rpl_1"
    );
  });
