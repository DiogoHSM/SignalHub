...
import { useSystemUsers, type UseSystemUsersResult } from "./useSystem";
...
export function SystemScreen({ ctx }: { ctx: ScreenCtx }) {
  ...
  const consoleUsers = useSystemUsers({ client: ctx.client, enabled: ctx.user?.isAdmin === true });
  ...
  return (
    <>
      ...
      {ctx.user?.isAdmin ? (
        <ConsoleAccessSection currentUser={ctx.user} users={consoleUsers} pushToast={ctx.pushToast} />
      ) : null}
    </>
  );
}
