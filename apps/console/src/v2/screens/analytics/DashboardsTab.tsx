...
const SERIES_COLORS = ["var(--accent)", "#58a6ff", "#d2a8ff", "#ffa657"];
...
function ReportWidget({ widget }: { widget: DashboardReportWidget }) {
  ...
  if (value !== null) {
    return <div><div className="sh-faint" style={{ fontSize: 11 }}>{String(data.label ?? widget.title)}</div><strong style={{ fontSize: 28 }}>{value.toLocaleString()}</strong>{numeric(data.open) !== null ? <div className="sh-muted" style={{ fontSize: 11 }}>{numeric(data.open)} open</div> : null}</div>;
  }
  if (series.length > 0) {
    return <div style={{ display: "grid", gap: 10 }}>{series.map((item, index) => {
      const values = Array.isArray(item.values) ? item.values.filter((entry): entry is number => typeof entry === "number") : [];
      return <div key={`${String(item.key ?? item.label ?? index)}`}><div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}><span>{String(item.label ?? item.key ?? "Series")}</span><strong>{values.reduce((sum, entry) => sum + entry, 0).toLocaleString()}</strong></div><Sparkline data={values.length ? values : [0]} color={SERIES_COLORS[index % SERIES_COLORS.length]} height={52} /></div>;
    })}<span className="sh-faint" style={{ fontSize: 10 }}>{buckets.length} buckets</span></div>;
  }
  if (rows.length > 0) {
    return <div style={{ display: "grid", gap: 6 }}>{rows.slice(0, 6).map((row, index) => <div key={index} style={{ display: "flex", justifyContent: "space-between", gap: 12, fontSize: 12 }}><span>{String(row.name ?? row.label ?? "Item")}</span><strong>{String(row.total ?? row.value ?? "")}</strong></div>)}</div>;
  }
...
function WidgetEditor({ widget, index, count, insightName, onChange, onMove, onRemove }: {
  ...
  return <div className="sh-row" style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 110px auto", gap: 8, alignItems: "center", padding: 10 }}>
    <div style={{ minWidth: 0 }}>
      {legacy ? <><strong style={{ fontSize: 12 }}>{widget.title}</strong><div className="sh-faint" style={{ fontSize: 10 }}>Legacy widget · {widget.type} · preserved read-only</div></> : <><input aria-label={`Widget ${index + 1} title`} className="sh-input" value={widget.title} onChange={(event) => onChange({ ...widget, title: event.target.value })} /><div className="sh-faint" style={{ fontSize: 10, marginTop: 3 }}>Saved insight: {insightName ?? "Unavailable insight"}</div></>}
    </div>
  ...
  return <div className="dashboards-layout" style={{ display: "grid", gap: 12, alignItems: "start" }}>
    <section className="sh-card" aria-label="Dashboard library">
      ...
      <div className="sh-card__body flush">
        {state.status === "loading" ? <p role="status" className="sh-muted" style={{ padding: 14 }}>Loading dashboards…</p> : null}
        ...
        {state.dashboards.map((row) => <button key={row.id} type="button" onClick={() => choose(row)} aria-pressed={row.id === selectedId} className={`sh-listrow ${row.id === selectedId ? "active" : ""}`} style={{ width: "100%", textAlign: "left" }}><div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}><strong>{row.name}</strong><span className="sh-tag">{row.category}</span></div><div className="sh-faint" style={{ fontSize: 10.5 }}>{row.widgets.length} widgets · {row.filters.window ?? "24h"}</div></button>)}
      </div>
    </section>

    <section className="sh-card" aria-label="Dashboard editor">
      <div className="sh-card__head"><div><h2 className="sh-h2">{selectedId ? "Edit dashboard" : "New dashboard"}</h2><div className="sh-faint" style={{ fontSize: 11 }}>Compose reusable operational views</div></div></div>
      <div className="sh-card__body" style={{ display: "grid", gap: 12 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}><label><span className="sh-label">Category</span><select aria-label="Dashboard category" className="sh-input" value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value as DashboardForm["category"] })}><option value="operational">Operational</option><option value="executive">Executive</option><option value="product">Product</option></select></label><label><span className="sh-label">Default window</span><select aria-label="Dashboard window" className="sh-input" value={form.window} onChange={(event) => setForm({ ...form, window: event.target.value as DashboardForm["window"] })}><option value="24h">Last 24 hours</option><option value="7d">Last 7 days</option><option value="30d">Last 30 days</option></select></label></div>
        <div><div className="sh-label" style={{ marginBottom: 5 }}>Add saved insight</div><div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 6 }}><select aria-label="Saved insight" className="sh-input" value={insightToAdd} onChange={(event) => setInsightToAdd(event.target.value)}><option value="">Select an insight…</option>{state.insights.map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select><button className="sh-btn secondary" type="button" disabled={!insightToAdd} onClick={() => { const insight = state.insights.find((row) => row.id === insightToAdd); if (insight) setForm({ ...form, widgets: [...form.widgets, newInsightWidget(insight)] }); setInsightToAdd(""); }}><Icon name="plus" size={13} />Add</button></div>{state.insights.length === 0 ? <div className="sh-faint" style={{ marginTop: 5, fontSize: 10.5 }}>Create a saved trend before composing insight widgets.</div> : null}</div>
        <div style={{ display: "grid", gap: 6 }}>{form.widgets.length === 0 ? <EmptyHint icon="pulse" title="No widgets" sub="Add saved insights to compose this dashboard." /> : form.widgets.map((widget, index) => <WidgetEditor key={widget.id} widget={widget} index={index} count={form.widgets.length} insightName={insightId(widget) ? insightNames.get(insightId(widget)!) ?? null : null} onChange={(next) => setForm({ ...form, widgets: form.widgets.map((row, rowIndex) => rowIndex === index ? next : row) })} onMove={(delta) => setForm({ ...form, widgets: move(form.widgets, index, delta) })} onRemove={() => setForm({ ...form, widgets: form.widgets.filter((_, rowIndex) => rowIndex !== index) })} />)}</div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}><button className="sh-btn primary" type="button" disabled={state.busy} onClick={() => void save()}><Icon name="check" size={13} />{selectedId ? "Save changes" : "Create dashboard"}</button>{selectedId ? <><button className="sh-btn secondary" type="button" disabled={state.busy} onClick={() => void duplicate()}><Icon name="copy" size={13} />Duplicate</button><ConfirmButton label="Archive" confirmLabel="Confirm archive" icon="archive" kind="ghost" onConfirm={() => void archive()} /></> : null}</div>
      </div>
    </section>

    <section className="sh-card" aria-label="Dashboard preview">
      <div className="sh-card__head"><div><h2 className="sh-h2">Preview</h2><div className="sh-faint" style={{ fontSize: 11 }}>{selectedId ? `${form.window} report` : "Save before previewing"}</div></div>{selectedId ? <button aria-label="Refresh dashboard preview" className="sh-btn ghost icon" type="button" onClick={() => void state.previewDashboard(selectedId, form.window)}><Icon name="refresh" size={14} /></button> : null}</div>
      <div className="sh-card__body" style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 8 }}>
        {state.previewStatus === "idle" ? <div style={{ gridColumn: "1/-1" }}><EmptyHint icon="pulse" title="No preview loaded" sub={selectedId ? "Refresh to calculate every widget." : "Save the dashboard to generate a report."} /></div> : null}
        {state.previewStatus === "loading" ? <p role="status" className="sh-muted" style={{ gridColumn: "1/-1" }}>Calculating dashboard…</p> : null}
        {state.previewStatus === "error" ? <div style={{ gridColumn: "1/-1" }}><EmptyHint icon="alert" title="Preview failed" sub="The saved dashboard report could not be generated." /></div> : null}
        {state.previewStatus === "unavailable" ? <div style={{ gridColumn: "1/-1" }}><EmptyHint icon="alert" title="Reports unavailable" sub="This API does not expose dashboard reports." /></div> : null}
        {state.previewStatus === "ok" && state.preview?.widgets.length === 0 ? <div style={{ gridColumn: "1/-1" }}><EmptyHint icon="pulse" title="Empty report" sub="No widget results were returned." /></div> : null}
