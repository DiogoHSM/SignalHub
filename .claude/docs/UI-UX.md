...
- System Health surfaces a read-only "System needs attention" banner for degraded queue probes, disabled/failed retention, and stale or failed backups; the banner points operators back to logs, the hosting panel, and the doctor CLI rather than exposing secrets or local paths.
...
- The channel form's type selector covers four types: generic webhook, native Slack, native Discord, and email. Webhook/Slack/Discord share the same URL + optional secret header fields, with type-specific label and help text (for example, "Slack Incoming Webhook URL" pointing operators at the Slack app's Incoming Webhooks settings). The saved secret header value is write-only and never displayed after submission. For Slack and Discord specifically, the webhook URL itself is the credential (the token lives in the path), so it is write-only too: the console only ever shows a masked preview (origin + a few characters of the path), never the full URL, and leaving the URL field blank on edit keeps the existing one. Generic webhook URLs are not treated as secret and remain visible after creation.
...
- Notification channel rows expose compact edit actions. Editing reuses the channel form, keeps saved secret header values and (for Slack/Discord) the webhook URL write-only, and should update the channel row without forcing a full page refresh.
...
- Data governance retention copy must explain that saved project/environment values override installation defaults for that category whether shorter or longer; categories absent from the scoped policy use their installation defaults.
