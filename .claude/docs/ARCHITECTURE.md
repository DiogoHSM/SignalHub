...
2. Humans log in through `/auth/login` or the optional Google OAuth flow and receive a random 32-byte opaque session token. The cookie contains only that token; Postgres stores only its SHA-256 hash in `auth_sessions`.
...
Production human sessions use the opaque `__Host-sigmon_session` cookie with `Secure`, `HttpOnly`, `SameSite=Lax`, and `Path=/`. Active lookup requires an unexpired, unrevoked row for an unarchived user. Logout revokes the current row; password changes and user archival revoke all sessions for that user transactionally. The OAuth state cookie remains `sigmon_oauth_state` because it is intentionally scoped to `/auth/google/callback`, which is incompatible with the `__Host-` prefix.
...
- `analytics_insights`
- `analytics_promoted_event_properties`
- `event_rollup_hourly`
- `experiments`
- `surveys`
- `message_campaigns`
...
Opt-in browser click maps are stored in the `click_events` telemetry table. `POST /v1/clicks` accepts normalized viewport coordinates, viewport dimensions, route, safe selector, and optional element tag/role metadata. The browser SDK helper avoids text, values, DOM snapshots, screenshots, and form fields by design. `GET /query/events/click-map` aggregates click density by route, safe selector, and bounded grid bucket for Events investigation. `click_events` is owned by the scoped `clicks` category; only an absent or invalid clicks value falls back to the installation `RETENTION_EVENTS_DAYS` default. Click deletion totals aggregate into `deleted_events`.

Privacy-safe browser replays are stored as inline `jsonb` event arrays in the `session_replays` telemetry table. `POST /v1/replays` accepts a masked interaction timeline keyed by `replay_id`; errors and product events can include the same `replay_id`. Incident detail returns linked replay context for errors, `GET /query/replays/:replayId` returns the replay plus linked product-event markers for event investigation, and `GET /query/replays` lists replay samples filtered by saved segment, tenant, user, and event name. The v2 incident view carries the primary occurrence timestamp into the replay panel so the UI can compute and highlight the error offset while keeping stack and breadcrumb context beside the masked timeline. Replay payloads store route, safe selectors, normalized click coordinates, sanitized messages, and bounded event data only. Direct HTTP ingestion rejects a replay above 64 KiB of serialized UTF-8, more than 300 events, or event `data` above five container levels or 64 total object keys. The byte budget is measured once over the incoming payload, before schema normalization, with a UTF-8 encoder. Replays do not store screenshots, DOM snapshots, raw text, input values, passwords, cookies, or HTML. Inline storage remains intentionally simpler while this bounded payload fits one row and is read as one timeline; move to chunked replay storage only when supported replay fidelity or measured row/read pressure requires payloads beyond this budget. `session_replays` is owned by the scoped `replays` category; only an absent or invalid replays value falls back to the installation `RETENTION_EVENTS_DAYS` default. Replay deletion totals aggregate into `deleted_events`.
...
The scheduler role owns the retention scheduler. When `RETENTION_ENABLED=true`, it periodically deletes old telemetry from `events`, `click_events`, `session_replays`, `errors`, `traces`, `spans`, `llm_calls`, `web_vitals`, `profiles`, and `breadcrumbs`, and expires old `dead_letter_jobs` using configured retention windows and bounded batches. Each physical telemetry table has one category owner and one effective project/environment cutoff: a valid scoped category value replaces the installation default whether shorter or longer, while a missing policy/category or invalid legacy value uses the installation default. Cutoffs are exact elapsed 24-hour intervals and strict older-than deletion preserves rows exactly at the boundary. `events`, `click_events`, and `session_replays` are owned only by events, clicks, and replays respectively; clicks, replays, and web vitals inherit `RETENTION_EVENTS_DAYS` only when their scoped category is absent or invalid. Existing `retention_runs` counters aggregate click/replay deletions into `deleted_events` and retain separate web-vitals, profiles, and dead-letter counts.

The scheduler role also owns the event rollup job. When `EVENT_ROLLUPS_ENABLED=true`, it maintains a per-day, per-actor, per-event-name rollup in `event_actor_daily` so long-range retention queries stay fast without materializing raw events. Progress is tracked by a generic `event_rollup_state` watermark table (keyed by project, environment, and rollup name; the actor-daily rollup uses a sentinel global scope since it processes every project/environment in one pass). The job always reprocesses the trailing two days to absorb late-arriving telemetry, and upserts are idempotent (`events` is overwritten, not incremented). `EVENT_ROLLUPS_LOOKBACK_DAYS` (default 400) bounds first-run backfill and must cover the longest effective scoped events window or cohort range operators expect to query, not merely `RETENTION_EVENTS_DAYS`. The rollup table is not selected by raw telemetry retention and outlives deleted `events` rows.

The scheduler role also owns warehouse exports. Project/environment-scoped `warehouse_destinations` select events, errors, traces, LLM calls, user profiles, and tenant profiles and store durable per-dataset cursors. Append-only telemetry advances incrementally by timestamp and id. Mutable identity profiles use a bounded cyclic snapshot ordered by actor id: after the final page, the profile cursor resets and the next scheduled pass starts another idempotent sweep. A profile changed behind the current page is therefore refreshed on the next cycle without adding a global lock to ingestion. Export runs write bounded batches into the external Postgres landing table `sigmon_telemetry_export` with one parameterized multi-row idempotent upsert per non-empty dataset, and each attempt is recorded in `warehouse_export_runs` for operator audit and retry visibility.
...
HTTP and heartbeat monitors live in `monitors`; individual probe and check-in history lives in `monitor_checks`. Admin monitor routes create and manage monitor definitions. Heartbeat monitors return a one-time `shhb_...` secret on creation; only the hash is stored. `POST /v1/heartbeats/:id` accepts a check-in only while the monitor, its environment, and its project are all active. An archive at any of those levels returns the same `404 heartbeat_monitor_not_found`, while a bad secret against an otherwise active monitor remains `401 invalid_heartbeat_secret`. The write transaction revalidates the exact scope and serializes project, environment, and monitor lifecycle state before recording the check. Monitor down/recovery events are represented as `alert_events` with `monitor_id` set.
...
`GET /query/events/retention` analyzes temporal retention for a selected project/environment window. Cohorts are computed entirely in SQL as a cohort-by-period CTE, anchored on each actor's `user_profiles.first_seen_at` rather than the minimum entry-event timestamp inside the queried window, so an actor who existed before the window but re-fires the entry event inside it does not start a bogus new cohort. Retention is scoped to actors with a `user_profiles` row (identified via `user_id`); session/trace-only activity does not anchor a cohort. Operators pass an optional entry event (a cohort eligibility filter), an optional return event (absent means any event counts as retained), a period (`daily`, `weekly`, or `monthly`), an interval count, and an optional `range_days` (1..730) that overrides the `window`-derived range for long lookback queries. `RETENTION_EVENTS_DAYS` remains the installation-level raw-versus-rollup routing threshold; this query routing is not scope-aware. The scoped events category independently determines when raw `events` rows are deleted and may be shorter or longer. A shorter scoped window can therefore remove raw rows before a request crosses the rollup-routing threshold, leaving cohort results in that gap incomplete; a longer window can retain raw rows even when the query chooses rollups. `EVENT_ROLLUPS_LOOKBACK_DAYS` must cover every long-range cohort window operators rely on. The response reports `source: "raw" | "rollup"`.
...
Saved event trends live in `analytics_insights` and are scoped to one project/environment. A definition selects an hourly or daily UTC bucket, event count or exact unique-actor metric, optional event name, bounded property filters, and one optional breakdown. `GET /query/analytics/trends` executes either a saved `insight_id` or an explicit definition, zero-fills empty buckets, and caps breakdown output to the top 20 deterministic series. Stable actors use `user_id`, then `tenant_id`, `session_id`, and `trace_id`.

Breakdown properties are an explicit governance boundary. `/admin/analytics/promoted-properties` creates a project/environment-scoped expression index and records its lifecycle as `pending`, `building`, `ready`, or `failed`; only `ready` properties are offered as breakdowns. Property names use the same allowlist in admin and query parsing and are always bound through Kysely rather than interpolated as unchecked SQL. Archiving an in-use property is rejected, and promotion/archive operations share an advisory lock so index metadata cannot race the physical index.

`event_rollup_hourly` is the count fast path for older, unfiltered trend buckets. The worker refreshes bounded historical windows plus the current and previous UTC hour under the existing rollup advisory lock. Query execution is hybrid: eligible completed hours come from rollups while the recent tail, filtered trends, and any interval not proven complete use raw `events`. Missing breakdown values remain a first-class `(none)` series in both paths. Unique actors are deduplicated exactly across the complete requested window rather than summing hourly cardinalities.

Analytics dashboards may reference insights by id. Report rendering resolves each insight in the same project/environment and evaluates widgets independently; a missing or failed insight returns an error on that widget without discarding successful widgets in the report.
...
The dedicated Incident view uses `GET /query/incidents/error-groups/:id` with project, environment, and optional raw error scope. The incident repository returns the selected group, a primary occurrence, source-map resolution status, suggested priority, saved priority override, AI-ready code context, and two context collections. Additional raw occurrences are loaded from the cursor-paginated `GET /query/error-groups/:id/occurrences` endpoint, deduplicated in the client, and never repeat the primary occurrence. Strongly related context matches the incident by strong identifiers such as trace, session, user, tenant, and release. Nearby context is lower-confidence activity around the primary occurrence timestamp and is labeled separately so operators can use it as supporting context rather than direct causality.

The operations health rail loads project-level fleet summaries first and defers `GET /query/fleet/projects/:id/environments` until an operator expands a project. Generation guards discard responses after project-scope changes. Trace aggregate endpoints remain public and documented even when the current console does not render every aggregate directly.

The console production bundle keeps authentication, project/environment state, navigation, command palette, and the fleet rail in the entry chunk. Screen implementations load through four shared lazy groups: operations, observability, analytics/experimentation, and administration. Incident and tenant drill details reuse their owning group promise. A screen-level Suspense fallback uses the v2 empty-state language, and a retryable error boundary clears a rejected group promise before re-importing it. This grouping avoids both the previous monolithic entry and one-request-per-small-screen fragmentation.

The browser URL is the shareable source of truth for console location. Sections use `/console/:section`; incidents retain `/console/incidents/error-groups/:groupId`; tenant details use `/console/entities/tenants/:tenantId`; and `project_id` plus `environment_id` preserve scope. Initial loads and `popstate` restore section, detail, and scope after project data resolves. In-console navigation pushes history, scope changes replace the current scoped URL, and detail back navigation returns through history when the detail was opened in-console while direct deep links fall back to their owning section.
