...
Cohort-query non-goal: this change does not make `GET /query/events/retention` routing scope-aware. `RETENTION_EVENTS_DAYS` remains its installation-level raw-versus-rollup threshold, while the scoped events value independently controls raw-row deletion. A shorter scoped window can delete raw rows before cohort routing selects rollups, so ranges in that gap can be incomplete; a longer window can retain raw rows after routing selects rollups. Operators must size `EVENT_ROLLUPS_LOOKBACK_DAYS` for the longest effective scoped events window or cohort range they rely on. `event_actor_daily` is not selected by raw telemetry retention.
