...
Migration and rollback: before deployment, inventory every server-side identify integration, create a server-capability key, place it only in server secret storage, and switch each integration to that key. Validate the replacement with identify `202`; validate that a browser key is denied with `403` while ordinary browser telemetry remains `202`. After deployment run `pnpm privacy:redact-feedback-urls` once against the production database and retain its scanned/updated log for the release record; reads remain sanitized until the backfill has completed. A failed server integration is rolled back by restoring its previous secret reference only if it already points to a server-capability key; never reclassify or reuse an exposed legacy/browser key as a server credential. If an application rollback is necessary, keep the new server key in the server integration and roll back only the application release. URL query-value redaction is intentionally irreversible, and MCP raw-detail exposure is rolled back by setting `MCP_ALLOW_RAW_DETAIL=false` and restarting the MCP process.
