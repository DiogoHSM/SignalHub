...
- `apps/worker`: telemetry worker, scheduler jobs, and Postgres warehouse export writer.
- `packages/cli`: Node-based SignalMonitor CLI, currently focused on source-map CI uploads.
- `packages/mcp`: `@sigmon/mcp`, a stdio Model Context Protocol server (`@modelcontextprotocol/sdk`) exposing nine read-only `/query/*` investigation tools to coding agents, authenticated with a read token. Private, not published to npm.
